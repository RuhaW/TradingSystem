    ...
    */
    std::vector<BookUpdate> getData()
    {
        ...
        while (getline(file, line))
        {
            ...
            std::string ts = vec[0].substr(0, 4) + '-'
                    + vec[0].substr(4, 2) + '-'
                    + vec[0].substr(6, 2) + ' ';
            ts += vec[3].substr(0,8);
...
//            time_t epoch = dur.total_seconds();
            ...
            BookUpdate b1(0,std::stod(vec[4]),std::stol(vec[5]),"GAIN",true,vec[1].c_str(),epoch);
            BookUpdate b2(0,std::stod(vec[6]),std::stol(vec[7]),"GAIN",false,vec[1].c_str(),epoch);
